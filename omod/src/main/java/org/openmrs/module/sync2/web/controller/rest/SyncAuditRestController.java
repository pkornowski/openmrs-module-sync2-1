...
public class SyncAuditRestController {
    ...
    public String getMessagesList(
        ...
        String resource = extractResourceName(resourceNameEnum);
    ...
    private String extractAction(String enumValue) {
        ...
        try {
            return Action.valueOf(enumValue).name() == Action.PUSH_AND_PULL.name() ?
    ...
    private String extractResourceName(String enumValue) {
        try {
            return Resources.valueOf(enumValue).getName();
        } catch(IllegalArgumentException e) {
